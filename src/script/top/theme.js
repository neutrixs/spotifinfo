const themeChange = function(force){
    ...
    let replaceSelectedWith = isLight ? ['selectedLight','selected'] : ['selected','selectedLight']
    let selected = $(isLight ? '.selectedLight' : '.selected')

    if(force){
        modeJquery = 'addClass'
        modeVanilla = 'add'
        replaceWith = ['white','black']
        replaceSelectedWith = ['selected','selectedLight']
        selected = $('.selected')
    }
    ...
    for(let i=0;i<selected.length;i++){
        selected[i].classList.replace(replaceSelectedWith[0],replaceSelectedWith[1])
    }
