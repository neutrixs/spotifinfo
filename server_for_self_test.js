...
app.get(/^\//,(req,res)=>{
    ...
    switch(req.path){
        case '/':
            if(isLoggedOut){
                res.render('indexOut.ejs',{isLoggedOut:isLoggedOut,useMinify:false,version:version})
            }
            else{
                res.render('index.ejs',{isLoggedOut:isLoggedOut,useMinify:false,version:version})
            }
        return
        case '/account':
            if(isLoggedOut){
            ...
            res.render('account.ejs',{isLoggedOut:isLoggedOut,useMinify:false,version:version})
        return
        case '/top_tracks':
            if(isLoggedOut){
                res.redirect('/')
            }
            res.render('top_tracks.ejs',{isLoggedOut:isLoggedOut,useMinify:false,version:version})
        return
        case '/login':
